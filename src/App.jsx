    ...
    <div className="min-h-screen bg-gray-50 flex flex-col">
      ...
      <div className="flex-1 max-w-5xl mx-auto p-4 w-full">
        ...
        <header className="bg-white shadow p-4 rounded-lg mb-6">
          <div className="flex justify-between items-center max-w-6xl mx-auto">
            ...
            <nav className="hidden md:flex items-center gap-8 text-gray-700 font-medium">
              ...
              <Link className="hover:text-blue-600 transition-colors" to="/volunteer">Voluntariado</Link>
              ...
              {session ? (
                <>
                  <Link className="hover:text-blue-600 transition-colors" to="/admin">Painel</Link>
                  <Link className="hover:text-blue-600 transition-colors" to="/admin/estoque">Estoque</Link>
                  <Link className="hover:text-blue-600 transition-colors" to="/admin/voluntarios">Voluntários</Link>
                ...
                <Link className="bg-blue-600 text-white px-4 py-2 rounded-lg hover:bg-blue-700 transition shadow-sm" to="/auth">
                  Entrar
                </Link>
              )}
          ...
          {menuOpen && (
            <nav className="flex flex-col gap-3 mt-4 md:hidden bg-white rounded p-4 shadow">
              ...
              <Link to="/volunteer" onClick={() => setMenuOpen(false)}>Voluntariado</Link>
              <Link to="/live" onClick={() => setMenuOpen(false)}>Ao Vivo</Link>
              <Link to="/donate" onClick={() => setMenuOpen(false)}>Doações</Link>
              ...
              <details>
                ...
                <div className="ml-4 mt-2 flex flex-col gap-2">
                  <Link to="/rehearsals" onClick={() => setMenuOpen(false)}>Ensaios</Link>
                </div>
              </details>

              {session ? (
                <>
                  <Link to="/admin/rehearsals" onClick={() => setMenuOpen(false)}>Admin Louvor</Link>
                  <Link to="/admin" onClick={() => setMenuOpen(false)}>Admin</Link>
                  <Link to="/admin/estoque" onClick={() => setMenuOpen(false)}>Estoque</Link>
                  <Link to="/admin/voluntarios" onClick={() => setMenuOpen(false)}>Voluntários</Link>
        ...
        <Routes>
          ...
          <Route path="/live" element={<iframe title="YouTube Live" width="100%" height="480"
            src="https://www.youtube.com/embed/VrpsnKlyThg?si=DuI_5oD9wqYNhIQX" allowFullScreen />} />
      ...
      <footer className="bg-gray-800 text-gray-300 py-6 mt-auto w-full">
        <div className="max-w-6xl mx-auto px-6 flex flex-col md:flex-row justify-between items-center gap-4">
          <p className="text-center md:text-left">
            © {new Date().getFullYear()} Igreja Ad Fidelidade. Todos os direitos reservados.
          </p>
          <div className="flex gap-4">
            <a href="https://www.youtube.com/@ADFidelidadeLondres" target="_blank" className="hover:text-white">YouTube</a>
            <a href="https://www.instagram.com/adfidelidadelondres" target="_blank" className="hover:text-white">Instagram</a>
          </div>
        </div>
      </footer>
